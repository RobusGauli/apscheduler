...
class BaseScheduler(six.with_metaclass(ABCMeta)):
    ...
    _trigger_plugins = dict(
        (ep.name, ep) for ep in iter_entry_points("apscheduler.triggers")
    )
    _trigger_classes = {}
    _executor_plugins = dict(
        (ep.name, ep) for ep in iter_entry_points("apscheduler.executors")
    )
    _executor_classes = {}
    _jobstore_plugins = dict(
        (ep.name, ep) for ep in iter_entry_points("apscheduler.jobstores")
    )
    _jobstore_classes = {}
